#!/usr/bin/env bash
# PostToolUse hook for Write: ruff isort-fix + format, then bugs-only check + mypy.
# Auto-fixes are safe on freshly-written files (no existing diff to disturb).
...
if command -v ruff >/dev/null 2>&1; then
  ruff check "$f" --select=I --fix --quiet 2>/dev/null || true
  ruff format "$f" --quiet 2>/dev/null || true
fi
